...
主干叙事以 v0.4.14 的 AnySplat-only / RGB-only 维护版为语义基线；第 9 和第 10 节
补充 v0.4.15-v0.4.20 的 schedule/VCP 实验结论，用来说明后续参数移动的取舍。
...
### 3.5 blending forward：一 tile 一个 block，按像素并行
...
### 3.6 bucket buffers：forward 记录，backward 并行化
...
### 4.3 CUDA backward/optimizer 的调用链
...
### 4.6 不可见 Gaussian 也会更新 optimizer state
...
### 5.2 Densification 信息在哪里产生，在哪里消费
...
### 6.2 adaptive_density_control 的具体步骤
...
### 6.5 新 Gaussian 的 optimizer state
...
### 7.3 densification-stage pruning 细节
...
## 8. Morton 排序、Opacity Reset、背景 alpha 及其它小改动

这一节把不属于初始化、主 forward/backward、density-control policy 的工程和语义小改动集中放在一起。根据 `CHANGELOG.md`，第 8 点应覆盖下面这些内容。
...
### 9.3 后续 schedule 实验的结论

从 v0.4.15 到 v0.4.20，主要不是改代码，而是在保持 AnySplat-only、RGB-only、fused backend 语义的前提下移动训练预算、densification end、Morton end 和 VCP window：

| 版本 | 主要超参数变化 | 结论 |
| --- | --- | --- |
| v0.4.15 | 18k 内启用 15k/16k/17k VCP | Gaussian 数下降约 30k，速度小幅提升，质量基本正常 |
| v0.4.16 | VCP threshold 0.90 -> 0.85 | 基本中性，不作为推荐方向 |
| v0.4.17 | 17k schedule，density/Morton end 到 14k，VCP 14.5k 起 | 比 v0.4.15 快约 4.17s，质量轻微下降 |
| v0.4.18 | 16.5k schedule，density/Morton end 到 13.5k | 继续提速，质量下降开始可见但仍温和 |
| v0.4.19 | 16k schedule，density/Morton end 到 13k，VCP 13.5k 起 | 当前 16k 方向最好，均值训练约 84.90s |
...
- 训练步数可以从 30k 降到 18k/17k/16k，但不能无限压缩；15k bicycle 已经出现更明显质量下降。

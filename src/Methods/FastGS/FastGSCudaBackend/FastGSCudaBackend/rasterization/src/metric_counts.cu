...
// Shared sorting pipeline with pruning_scores — only the final accumulation kernel differs.
    ...
    const bool proper_antialiasing)
{
    ...
    // Reuse the identical preprocess kernel from pruning_scores.
    kernels::pruning_scores::preprocess_cu<<<
    ...
    int n_instances;
    cudaMemcpy(&n_instances, primitive_buffers.n_instances,
               sizeof(uint), cudaMemcpyDeviceToHost);
    ...
    cub::DeviceScan::ExclusiveSum(
        ...
        n_visible_primitives);
    CHECK_CUDA(config::debug, "cub::DeviceScan::ExclusiveSum")

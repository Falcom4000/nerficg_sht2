...
from FastGSCudaBackend import _C
...
def update_metric_counts(
    ...
    """Accumulates per-Gaussian high-error pixel counts in-place.

    counts    : (N,) float32, zeroed by caller, modified in-place.
    metric_map: (H*W,) int32 — 1 = high-error pixel, 0 = normal.
    """
    # metric_counts wrapper does not use bg_color — pass a dummy but keep tuple length compatible.
    # The C++ wrapper signature skips bg_color, so we unpack the settings manually.
    s = rasterizer_settings
    _C.metric_counts(
        counts,
        metric_map,
        means,

...
from Methods.FastGS.Model import FastGSModel
from Methods.FastGS.FastGSCudaBackend import diff_rasterize, rasterize, update_metric_counts, RasterizerSettings
...
def extract_settings(
    ...
    if not isinstance(view.camera, PerspectiveCamera):
        raise Framework.RendererError('FastGS renderer only supports perspective cameras')
...
class FastGSRenderer(BaseRenderer):
    """Wrapper around the FastGS CUDA rasterization backend."""

    def __init__(self, model: 'BaseModel') -> None:
        super().__init__(model, [FastGSModel])
        if not Framework.config.GLOBAL.GPU_INDICES:
            raise Framework.RendererError('FastGS renderer not implemented in CPU mode')
        if len(Framework.config.GLOBAL.GPU_INDICES) > 1:
            Logger.log_warning(
                f'FastGS renderer not implemented in multi-GPU mode: '
                f'using GPU {Framework.config.GLOBAL.GPU_INDICES[0]}'
            )

    def render_image(self, view: View, to_chw: bool = False, benchmark: bool = False) -> dict[str, torch.Tensor]:
        if benchmark:
            return self.render_image_benchmark(view, to_chw=True)
    ...
    def render_image_training(
        ...
        image = diff_rasterize(
            ...
            densification_info=(
                self.model.gaussians.densification_info
                if update_densification_info
                else torch.empty(0)
            ),
            rasterizer_settings=extract_settings(
                view, self.model.gaussians.active_sh_bases, bg_color, self.PROPER_ANTIALIASING),
        )
        return image
    ...
    def render_image_inference(self, view: View, to_chw: bool = False) -> dict[str, torch.Tensor]:
        image = diff_rasterize(
            ...
            rasterizer_settings=extract_settings(
                view, self.model.gaussians.active_sh_bases,
                view.camera.background_color, self.PROPER_ANTIALIASING),
        )
    ...
    def render_for_scoring(
        ...
        """Accumulates per-Gaussian high-error pixel counts into metric_counts (in-place).

        metric_map   : (H*W,) int32 — 1 = high-error pixel.
        metric_counts: (N,)   float32, zeroed by caller, modified in-place.
        """

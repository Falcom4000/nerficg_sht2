...
namespace faster_gs::rasterization::kernels {
    ...
    __device__ inline mat3x3 convert_normalized_quaternion_to_rotation_matrix(
    ...
    __device__ inline float4 convert_normalized_quaternion_to_rotation_matrix_backward(
        const float4& quaternion,
        const mat3x3& dL_dR)
    {
        auto [r, x, y, z] = quaternion;
        ...
        return {
            2.0f * (x * dL_drx + y * dL_dry + z * dL_drz),
            2.0f * (r * dL_drx - 2.0f * x * dL_dxx + y * dL_dxy + z * dL_dxz),
            2.0f * (r * dL_dry + x * dL_dxy - 2.0f * y * dL_dyy + z * dL_dyz),
            2.0f * (r * dL_drz + x * dL_dxz + y * dL_dyz - 2.0f * z * dL_dzz)
        };
    }
        ...
        const uint total_sh_bases_rest)
    {
        ...
        const float3 grad_color = make_float3(
            grad_colors[primitive_idx],
            grad_colors[n_primitives + primitive_idx],
            grad_colors[2 * n_primitives + primitive_idx]
        );

"""FasterGSBasisRapid/Renderer.py"""
...
class FasterGSBasisRapidRenderer(BaseRenderer):
    ...
    def __init__(self, model: 'BaseModel') -> None:
        super().__init__(model, [FasterGSBasisRapidModel])
        if not Framework.config.GLOBAL.GPU_INDICES:
            raise Framework.RendererError('FasterGSBasisRapid renderer not implemented in CPU mode')
        if len(Framework.config.GLOBAL.GPU_INDICES) > 1:
            Logger.log_warning(f'FasterGSBasisRapid renderer not implemented in multi-GPU mode: using GPU {Framework.config.GLOBAL.GPU_INDICES[0]}')

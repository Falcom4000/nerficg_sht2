...
namespace faster_gs::rasterization::kernels::forward {
    ...
    struct MaxUint {
        __host__ __device__ __forceinline__ uint operator()(uint a, uint b) const {
            return a > b ? a : b;
        }
    };
    ...
    __global__ void preprocess_cu(
        ...
        float* __restrict__ primitive_inv_depth,
    ...
    __global__ void __launch_bounds__(config::block_size_blend) blend_cu(
        ...
        float* __restrict__ metric_counts,
        ...
        float* __restrict__ bucket_inv_depth,
        const uint width,
        const uint height,
        const uint grid_width)
    {
        ...
        __shared__ float collected_inv_depth[config::block_size_blend];
            ...
            for (int j = 0; !done && j < current_batch_size; ++j) {
                ...
                if (metric_counts != nullptr) {
                    ...
                    if (metric_map[pixel_idx]) {
                        atomicAdd(&metric_counts[collected_primitive_idx[j]], 1.0f);
                    }
                }
